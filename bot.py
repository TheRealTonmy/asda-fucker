...
# Dynamic Store IDs loaded from .env
...
def create_digital_label(code_string: str, product_name: str, new_price_str: str, old_price_str: str, image_url: Optional[str], filename: str = "label.png") -> discord.File:
    ...
    canvas = Image.new('RGB', (canvas_w, canvas_h), '#F0F2F5') 
    draw = ImageDraw.Draw(canvas)

    try:
        # Massively increased font sizes for the 1000x1250 canvas
        font_banner = ImageFont.truetype("OpenSans-Bold.ttf", 85)
        font_title = ImageFont.truetype("OpenSans-Bold.ttf", 65)
        font_was = ImageFont.truetype("OpenSans-Regular.ttf", 55)
        font_price = ImageFont.truetype("OpenSans-Bold.ttf", 170) 
    except IOError:
        logging.error("CRITICAL: OpenSans fonts not found in the directory! Text will be tiny.")
        font_banner = font_title = font_was = font_price = ImageFont.load_default()

    # Dynamic Red Header - Made taller to fit the huge text
    draw.rectangle([0, 0, canvas_w, 130], fill="#E51937")
    draw.text((canvas_w / 2, 65), "CLEARANCE", fill="white", font=font_banner, anchor="mm", weight="bold")
    current_y = 180

    # Image Processing (Unchanged size)
    if image_url:
        try:
            ...
            if resp.status_code == 200:
                ...
                draw.rounded_rectangle(
                    ...
                    radius=20, fill="white", outline="#D0D5DD", width=2
                )
                
                canvas.paste(prod_img, (img_x, current_y), prod_img)
                current_y += prod_img.height + 80
        except Exception as e:
            logging.error(f"Image load failed: {e}")

    # Bold Centered Title - Adjusted wrap width for larger font
    wrapped_title = textwrap.fill(product_name, width=28)
    draw.multiline_text((canvas_w / 2, current_y), wrapped_title, fill="#111827", font=font_title, anchor="ma", align="center", spacing=15)
    text_bbox = draw.multiline_textbbox((canvas_w / 2, current_y), wrapped_title, font=font_title, anchor="ma", spacing=15)
    current_y = text_bbox[3] + 60

    # WAS Price with thicker strike-through
    was_text = f"WAS {old_price_str}"
    was_bbox = draw.textbbox((canvas_w / 2, current_y), was_text, font=font_was, anchor="mm")
    draw.text((canvas_w / 2, current_y), was_text, fill="#6B7280", font=font_was, anchor="mm")
    draw.line([was_bbox[0] - 20, current_y, was_bbox[2] + 20, current_y], fill="#E51937", width=7)
    current_y += 70

    # NOW Price Box - Made much larger to hold the 170px price font
    box_w, box_h = 550, 240
    box_x = int((canvas_w - box_w) / 2)
    
    draw.rounded_rectangle([box_x, current_y, box_x + box_w, current_y + box_h], radius=35, fill="#FFCC00", outline="#E5B700", width=6)
    draw.text((canvas_w / 2, current_y + (box_h / 2)), new_price_str, fill="#111827", font=font_price, anchor="mm")
    current_y += box_h + 100

    # Barcode Generation - Kept exactly the same size as requested, text removed
    rv = io.BytesIO()
    ...
    if bc_img.width > canvas_w - 100:
        bc_img.thumbnail((canvas_w - 100, 300), Image.Resampling.LANCZOS)
