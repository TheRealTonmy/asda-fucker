...
def create_digital_label(code_string: str, product_name: str, new_price_str: str, old_price_str: str, image_url: Optional[str], filename: str = "label.png") -> discord.File:
    """Aesthetically upgraded layout for the final clearance label."""
    canvas_w, canvas_h = 1250, 1400
    canvas = Image.new('RGB', (canvas_w, canvas_h), '#F0F2F5') # Softer, modern gray background
    draw = ImageDraw.Draw(canvas)

    # Clean typography
    try:
        font_banner = ImageFont.truetype("arialbd.ttf", 55)
        font_title = ImageFont.truetype("arialbd.ttf", 46)
        font_was = ImageFont.truetype("arial.ttf", 38)
        font_price = ImageFont.truetype("arialbd.ttf", 95) 
    except IOError:
        font_banner = font_title = font_was = font_price = ImageFont.load_default()

    # Dynamic Red Header
    draw.rectangle([0, 0, canvas_w, 90], fill="#E51937")
    draw.text((canvas_w / 2, 45), "CLEARANCE", fill="white", font=font_banner, anchor="mm", weight="bold")
    current_y = 140

    # Image Processing with rounded shadow box
    if image_url:
        try:
            ...
            if resp.status_code == 200:
                ...
                current_y += prod_img.height + 70
        except Exception as e:
            logging.error(f"Image load failed: {e}")

    # Bold Centered Title
    wrapped_title = textwrap.fill(product_name, width=30)
    draw.multiline_text((canvas_w / 2, current_y), wrapped_title, fill="#111827", font=font_title, anchor="ma", align="center", spacing=10)
    text_bbox = draw.multiline_textbbox((canvas_w / 2, current_y), wrapped_title, font=font_title, anchor="ma", spacing=10)
    current_y = text_bbox[3] + 50

    # WAS Price with crisp strike-through
    ...
    draw.line([was_bbox[0] - 15, current_y, was_bbox[2] + 15, current_y], fill="#E51937", width=5)
    current_y += 60

    # NOW Price Box (Punchy Yellow)
    box_w, box_h = 420, 160
    box_x = int((canvas_w - box_w) / 2)
    
    draw.rounded_rectangle([box_x, current_y, box_x + box_w, current_y + box_h], radius=25, fill="#FFCC00", outline="#E5B700", width=4)
    draw.text((canvas_w / 2, current_y + (box_h / 2)), new_price_str, fill="#111827", font=font_price, anchor="mm")
    current_y += box_h + 80

    # Barcode Generation (Numbers strictly removed via write_text=False)
    rv = io.BytesIO()
    pybarcode.get('code128', code_string, writer=ImageWriter()).write(
        rv, 
        options={
            "write_text": False,  # Text disabled per requirement
            "module_height": 22.0, 
            "quiet_zone": 2.0
        }
    )
